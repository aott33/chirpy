...
const upgradeUserRedChirpy = `-- name: UpgradeUserRedChirpy :one
UPDATE users
...
func (q *Queries) UpgradeUserRedChirpy(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, upgradeUserRedChirpy, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
